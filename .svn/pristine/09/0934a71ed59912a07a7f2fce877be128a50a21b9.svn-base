...
# rush to change it.
output_ruleset = 0 # 1 = output ruleset as csv
show_context = 0
minimum_level = 1
show_immediately = 0
...
csv_content = [] # csv output content
csv_filename = None # csv output filename
csv_writer = None
...
def whatever(source):
    print("souce string : " + source)

def custom(hit):
    args = hit.args
    length = len(hit.parameters)-1
    for k,v in args.items():
        if(k <= length):
            function_name = getattr(sys.modules[__name__], v)
            function_name(hit.parameters[k])

def testCustom():
    hit = Hit(c_ruleset["test"])
    hit.args = {1:"whatever"}
    hit.parameters = ["","testWhatever"]
    custom(hit)
...
def process_options():
    ...
    try:
        ...
        optlist, args = getopt.getopt(sys.argv[1:], "ce:m:nih?CSDQHIFP:", [
            ...
            "loadhitlist=", "savehitlist=", "diffhitlist=", "version", "help","outputrule"
        ])
        for (opt, value) in optlist:
            if opt == "--context" or opt == "-c":
                ...
                quiet = 1
                showheading = 0
                csv_content.append(['File', 'Line', 'Column', 'Level', 'Category', 'Name', 'Warning',
                                    'Suggestion', 'Note', 'CWEs', 'Context', 'Fingerprint'])
...
def preprocess_files_c(files,patch_infos):
    """
    ...
    for f in files:
        if (not allowlink) and os.path.islink(f):
            ...
            if (skipdotdir and len(base_filename) > 1
                ...
                return
            for dir_entry in os.listdir(f):
                preprocess_files_c([os.path.join(f,dir_entry)],patch_infos)
            # Now we will FIRST check if the file appears to be a C/C++ file, and
            # THEN check if it's a regular file or symlink.  This is more complicated,
            # but I do it this way so that there won't be a lot of pointless
            # warnings about skipping files we wouldn't have used anyway.
...
def process_files():
    ...
    if loadhitlist:
        ...
        if not files:
            ...
            return None
        preprocess_files_c(files,patch_infos)
...
def save_ruleset():
    csv_rule_filename = "ruleset_temp.csv"
    rule_format = [["key","hook", "level", "warning", "suggestion", "category","url", "other"]]
    for k,v in c_ruleset.items():
        tup_key = (k,)
        tup_value = (v[0].__name__,) +v[1:-1] + (str(v[-1]),)
        temp = tup_key + tup_value
        rule_format.append(list(temp))

    with open(csv_rule_filename, 'w+') as csv_file:
        writer = csv.writer(csv_file)
        for row in rule_format:
            writer.writerow(row)

def load_ruleset():
    csv_rule_custom = "ruleset_custom.csv"
    custom_rules = []
    with open(csv_rule_custom,"r+") as csv_file:
        reader = csv.reader(csv_file)
        for item in reader:
            custom_rules.append(item)
    for row in custom_rules[1:]:
        key = row[0]
        function_name = getattr(sys.modules[__name__], row[1])
        row[2] = int(row[2])
        other_dict = eval(row[-1])
        data_tuple = (function_name,) + tuple(row[2:-1]) + (other_dict,)
        c_ruleset[key] = data_tuple
